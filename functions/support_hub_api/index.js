...
app.get('/api/dashboard', async (req, res) => {
	try {
		...
		const [pR, dR, tR] = await Promise.all([
			...
			q(c, "SELECT ROWID, Name, Role,DevStatus, Skills, Email FROM Developers"),
		...
		res.json({ status: 'success', data: {
				...
				developers: dR.map(r => ({ id: r.Developers.ROWID, name: r.Developers.Name, avatar: r.Developers.Name.split(' ').map(w=>w[0]).join('').slice(0,2).toUpperCase(), role: r.Developers.Role, status: r.Developers.DevStatus, skills: r.Developers.Skills ? r.Developers.Skills.split(',').map(s=>s.trim()) : [] })),
...
app.post('/api/developers', async (req, res) => {
	try {
		...
		if (!name) return res.status(400).json({ status:'error', message:'Name required' });
		const row = await tbl(c, 'Developers').insertRow({ Name: name, Role: role||'Dev',const row = await tbl(c, 'Developers').insertRow({ Name: name, Role: role||'Dev', DevStatus:'Active', Skills: Array.isArray(skills)?skills.join(','):(skills||''), Email: email||'' }); Status:'active', Skills: Array.isArray(skills)?skills.join(','):(skills||''), Email: email||'' });
		res.status(201).json({ status:'success', data: { id: row.ROWID } });
